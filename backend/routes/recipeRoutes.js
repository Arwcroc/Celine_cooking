...
router.post('/', upload.single('image'), (req, res) => {
    ...
    const query = 'INSERT INTO recipes (title, image, servings, ingredients, steps, diet, tags) VALUES (?, ?, ?, ?, ?, ?, ?)';
    db.query(query, [title, image, servings, JSON.stringify(parsedIngredients), JSON.stringify(parsedSteps), diet, JSON.stringify(parsedTags)], (err, result) => {
        if (err) {
...
router.put('/:id', upload.single('image'), (req, res) => {
    ...
    db.query(selectImageSql, [recipeId], (err, results) => {
        if (err) {
			console.error(err)
            return res.status(400).json({ error: `Failed to fetch current image: ${err}` });
        }

        image = req.file ? req.file.filename : results[0].image;
		
		const sql = 'UPDATE recipes SET title = ?, image = ?, ingredients = ?, steps = ?, servings = ?, diet = ?, tags = ? WHERE id = ?';
        db.query(sql, [title, image, ingredients, steps, servings, diet, tags, recipeId], (err, result) => {
            if (err) {
				console.error(err)
                return res.status(400).json({ error: `Failed to update recipe: ${err}` });
            }
			deleteUnusedImages()
